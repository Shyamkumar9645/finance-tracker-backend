...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    try {
      ...
      // All transactions are for pittala_eeru
      const pittala_eeruTransactions = [
        {
            user_id: 1,
            person_id: personId,
            amount: 10000.00,
      ...
      for (const transaction of pittala_eeruTransactions) {
      ...
      console.log(`Inserted ${insertedCount} transactions for pittala_eeru, skipped ${skippedCount} existing transactions.`);
  ...
  async down(queryInterface, Sequelize) {
    try {
      // Get pittala_eeru's ID
      const pittala_eeru = await queryInterface.sequelize.query(
        `SELECT id FROM people WHERE name = 'pittala_eeru' LIMIT 1`,
        { type: queryInterface.sequelize.QueryTypes.SELECT }
      );

      if (!pittala_eeru || pittala_eeru.length === 0) {
        console.log("pittala_eeru not found, nothing to delete");
        return Promise.resolve();
      }

      const personId = pittala_eeru[0].id;

      // Delete transactions for pittala_eeru
      const deleteResult = await queryInterface.bulkDelete('transactions', {
        person_id: personId
      });

      console.log(`Deleted ${deleteResult} transactions for pittala_eeru`);
      return Promise.resolve();
    } catch (error) {
